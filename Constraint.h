...
class SHAPEOP_API Constraint {
 public:
  /** \brief Creates a constraint from a string type, a number of indices, a weight and the initial point positions.
   ...
   \return A std::shared pointer to the Constraint, which is empty if failed.
   */
...
class SHAPEOP_API EdgeStrainConstraint : public Constraint {
 public:
  /** \brief Constraint constructor. The target length is set to the distance of the two vertices in the parameter positions.
      The parameters rangeMin and rangeMax can be used to specify a target range for the distance [rangeMin*distance,rangeMax*distance]
...
///////////////////////////////////////////////////////////////////////////////
/** \brief Triangle strain constraint. TODO See \cite Bouaziz2014 for more details.*/
class SHAPEOP_API TriangleStrainConstraint : public Constraint {
 public:
  /** \brief Constraint constructor. TODO
  \param idI are three indices of the vertices of the triangle
  \param weight The weight of the constraint to be added relative to the other constraints.
  \param positions The positions of all the n vertices stacked in a 3 by n matrix.
  \param rangeMin The factor to determine the minimal strain: rangeMin*strain
  \param rangeMax The factor to determine the maximal strain: rangeMax*strain
  */
...
///////////////////////////////////////////////////////////////////////////////
/** \brief Tetrahedron strain constraint. TODO See \cite Bouaziz2014 for more details.*/
...
class SHAPEOP_API AreaConstraint : public Constraint {
 public:
  /** \brief Constraint constructor. The target area is set to the area spanned by three vertices in the parameter positions.
      The parameters rangeMin and rangeMax can be used to specify a target range for the area [rangeMin*area,rangeMax*area]
    \param idI are three indices of the vertices forming the constrained triangle
    \param weight The weight of the constraint to be added relative to the other constraints.
    \param positions The positions of all the n vertices stacked in a 3 by n matrix.
    \param rangeMin The factor to determine the minimal distance from the target length: rangeMin*area
    \param rangeMax The factor to determine the maximal distance from the target length: rangeMax*area
  */
...
class SHAPEOP_API VolumeConstraint : public Constraint {
 public:
  /** \brief Constraint constructor. The target area is set to the volume spanned by four vertices in the parameter positions.
      The parameters rangeMin and rangeMax can be used to specify a target range for the volume [rangeMin*volume,rangeMax*volume]
    \param idI are four indices of the vertices of the tetrahedron
    \param weight The weight of the constraint to be added relative to the other constraints.
    \param positions The positions of all the n vertices stacked in a 3 by n matrix.
    \param rangeMin The factor to determine the minimal distance from the target length: rangeMin*volume
    \param rangeMax The factor to determine the maximal distance from the target length: rangeMax*volume
  */
...
class SHAPEOP_API BendingConstraint : public Constraint {
 public:
  /** \brief Constraint constructor. (TODO: is this true Sofien?) The target bend is set to the oen between the triangles spanned by four vertices in the parameter positions. The parameters rangeMin and rangeMax can be used to specify a target range for the bend [rangeMin*bend,rangeMax*bend]
    ...
    \param weight The weight of the constraint to be added relative to the other constraints.
    \param positions The positions of all the n vertices stacked in a 3 by n matrix.
    \param rangeMin The factor to determine the minimal distance from the target length: rangeMin*bend
    \param rangeMax The factor to determine the maximal distance from the target length: rangeMax*bend
  */
...
///////////////////////////////////////////////////////////////////////////////
/** \brief Laplacian for Fairing Constraint. Minimizes the uniform laplacian respectively the uniform laplacian of displacements.*/
class SHAPEOP_API TilingLaplacianConstraint : public Constraint {
 public:
  /** \brief Constraint constructor.
  \param idI The first index corresponds to the central vertex, while the remaining ones correspond to the one-ring neighborhood.
  \param weight The weight of the constraint to be added relative to the other constraints.
  ...
  \param displacement_lap If false the laplacian is minimized, if true, the laplacian of deformations w.r.t. initial positions.
  */
  TilingLaplacianConstraint(const std::vector<int> &idI,
                             Scalar weight,
                             const Matrix3X &positions,
                             bool displacement_lap);
  virtual ~TilingLaplacianConstraint() {}
  /** \brief Find the closest configuration from the input positions that satisfy the constraint.*/
  virtual void project(const Matrix3X & /*positions*/, Matrix3X &projections) const override final;
  /** \brief Add the constraint to the linear system.*/
  virtual void addConstraint(std::vector<Triplet> &triplets, int &idO) const override final;
...
///////////////////////////////////////////////////////////////////////////////
/** \brief Angle range constraint. Constrains the angle fromed by three points to a range. See \cite Deng et al. 2015 for more details.*/
class SHAPEOP_API AngleConstraint : public Constraint {
 public:
  /** \brief Constraint constructor. The target length is set to the distance of the two vertices in the parameter positions.
      The parameters rangeMin and rangeMax can be used to specify a target range for the distance [rangeMin*distance,rangeMax*distance]
    \param idI are three indices of the vertices that form the angle, with the first index corresponding to the 'tip'.
    \param weight The weight of the constraint to be added relative to the other constraints.
    \param positions The positions of all the n vertices stacked in a 3 by n matrix.
...
///////////////////////////////////////////////////////////////////////////////
/** \brief Mesh closeness constraint. Constrains a vertex to a position in space. Projects onto a given rest position.*/
class SHAPEOP_API MeshClosenessConstraint : public Constraint {
 public:
  ...
  \param positions The positions of all the n vertices stacked in a 3 by n matrix.
  */
  MeshClosenessConstraint(const std::vector<int> &idI,
                      Scalar weight,
                      const Matrix3X &positions);
  ...
  /** \brief Add the constraint to the linear system.*/
  virtual void addConstraint(std::vector<Triplet> &triplets, int &idO) const override final;
  /** \brief Set the AABB Tree.*/
  void setMesh(const std::shared_ptr<TrimeshSearcher<Matrix3X, Matrix3X>> &aabb_tree);
 private:
  //mutable TrimeshSearcher<Matrix3X, Matrix3X> searcher_;
  std::shared_ptr<TrimeshSearcher<Matrix3X, Matrix3X>> searcher_;
};
///////////////////////////////////////////////////////////////////////////////
} // namespace ShapeOp
///////////////////////////////////////////////////////////////////////////////
