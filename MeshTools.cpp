 ...
 */
Eigen::Vector3d MeshTools::zPlaneRot(double _angle, Eigen::Vector3d _p, Eigen::Vector3d _rot_center) {
   double a(_angle);
   Eigen::Matrix3d M;
   M << cos(a), -sin(a), 0.0,
         sin(a), cos(a), 0.0,
 ...
 */
void MeshTools::generateQuadRect(PolyMesh* mesh, size_t _width, size_t _height, bool _tessellate) {
 ...
 * @param _tessellate Specify whether the mesh should apply hinge tessellation
 */
void MeshTools::generateTriHex(TriMesh *mesh, size_t _radius, bool _tessellate) {
   size_t r(_radius);
   bool t(_tessellate);
