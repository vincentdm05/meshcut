...
SHAPEOP_INLINE std::shared_ptr<Constraint> Constraint::shapeConstraintFactory(const std::string &constraintType, const std::vector<int> &idI, Scalar weight, const Matrix3X &positions) {
  ...
  if (constraintType.compare("AngleConstraint") == 0) {  if (n !=  3) { return c; } return std::make_shared<AngleConstraint>(idI, weight, positions); }
  if (constraintType.compare("MeshClosenessConstraint") == 0) {  return std::make_shared<MeshClosenessConstraint>(idI, weight, positions); }
  return c;
}
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE TilingLaplacianConstraint::TilingLaplacianConstraint(const std::vector<int> &idI,
                                                                      Scalar weight,
                                                                      const Matrix3X &positions,
                                                                      bool displacement_lap) :
  Constraint(idI, weight) {
  weighted_rest_.setZero();
  if (displacement_lap) {
    Vector3 init_baryc = positions.col(idI[0])+positions.col(idI[1])+positions.col(idI[2]);
    int n_idx = static_cast<int>(idI.size()) /3;
    for (int i = 1; i < n_idx; ++i)
        for(int j = 0; j<3; ++j)
            weighted_rest_ += positions.col(idI[i*3+j]);
    weighted_rest_ /= double(n_idx - 1);
    weighted_rest_ -= init_baryc;
    weighted_rest_ *= weight_;
  }
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void TilingLaplacianConstraint::project(const Matrix3X & /*positions*/, Matrix3X &projections) const {
  projections.col(idO_) = weighted_rest_;
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void TilingLaplacianConstraint::addConstraint(std::vector<Triplet> &triplets, int &idO) const {
  idO_ = idO;
  triplets.push_back(Triplet(idO_, idI_[0], -weight_));
  triplets.push_back(Triplet(idO_, idI_[1], -weight_));
  triplets.push_back(Triplet(idO_, idI_[2], -weight_));
  double c = weight_ / ((idI_.size() - 3)/3);
  for (int i = 3; i < static_cast<int>(idI_.size()); ++i)
    triplets.push_back(Triplet(idO_, idI_[i], c));
  ++idO;
}
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE bool isFinite(const Vector3 &v) {
  return std::isfinite(v(0)) && std::isfinite(v(1)) && std::isfinite(v(2));
}

SHAPEOP_INLINE double clamp_sine_cosine(double val) {
  return (std::max)((std::min)(val, 1.0), -1.0);
}
...
SHAPEOP_INLINE void AngleConstraint::project(const Matrix3X &positions, Matrix3X &projections) const {
  ...
  if (isFinite(unit_v1) && isFinite(unit_v2)) {
    // cosine value of the angle gamma between v1 and v2
    double cos_gamma = clamp_sine_cosine(unit_v1.dot(unit_v2));
...
SHAPEOP_INLINE void AngleConstraint::setMinAngle(Scalar minAngle) {
  ...
  // Use parentheses to avoid coflicts with the min/max macros from windows.h
  minAngle_ = (std::max)(minAngle, 0.0);
  minAngleCos_ = clamp_sine_cosine(std::cos(minAngle_));
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void AngleConstraint::setMaxAngle(Scalar maxAngle) {
  maxAngle_ = (std::min)(maxAngle, M_PI);
  maxAngleCos_ = clamp_sine_cosine(std::cos(maxAngle_));
}
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE MeshClosenessConstraint::MeshClosenessConstraint(const std::vector<int> &idI,
                                                        Scalar weight,
                                                        const Matrix3X &positions) :
  Constraint(idI, weight) {
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void MeshClosenessConstraint::project(const Matrix3X & positions, Matrix3X &projections) const {
  // p is the input point
  Vector3 p = positions.col(idI_[0]);
  // q is the closest point on the input mesh
  Vector3 q;
  searcher_->closest_point(p, q);
  projections.col(idO_) = q * weight_;
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void MeshClosenessConstraint::addConstraint(std::vector<Triplet> &triplets, int &idO) const {
  idO_ = idO;
  triplets.push_back(Triplet(idO_, idI_[0], weight_));
  idO += 1;
}
///////////////////////////////////////////////////////////////////////////////
/*SHAPEOP_INLINE void MeshClosenessConstraint::setMesh(const Matrix3X &vertices, const Matrix3X &faces) {
* searcher_.build(vertices, faces); //umesto ovoga iskopiramo pointer
*}
*/
SHAPEOP_INLINE void MeshClosenessConstraint::setMesh(const std::shared_ptr<TrimeshSearcher<Matrix3X, Matrix3X> > &aabb_tree) {
 searcher_= aabb_tree;
}
///////////////////////////////////////////////////////////////////////////////
} // namespace ShapeOp
